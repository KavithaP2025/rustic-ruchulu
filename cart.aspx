<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="cart.aspx.cs" Inherits="RusticRuchulu.cart" %>
...
<body ng-controller="rusticRuchuluController" ng-init="GetCartItemsByUser(); GetUserAddress();">
                        ...
                        <nav class="site-navigation text-right" role="navigation">
                            <div class="">
                                ...
                                <ul class="site-menu main-menu js-clone-nav d-none d-lg-block">
                                    ...
                                    <%--User Cart--%>
                                    <li ng-if="IsLogin && LoginUserRole == 'User'"><a href="cart.aspx" class="nav-link active">
                                        <svg width="25" height="25" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" transform="rotate(0 0 0)">
    ...
    <section id="cart" class="section light-bg">
        <div class="p_0_24">
            <h2 class="mt_40">Cart</h2>
            <div class="row mt_12">
                <div class="col-xl-8 col-lg-8 col-sm-12">
                    <div class="cart-items-box">
                        ...
                        <div class="cart-body">
                            <div class="row cart-body-item" ng-repeat="cartItem in CartItems">
                                <div class="col-xl-2 col-lg-2 col-sm-12">
                                    <img ng-src="{{cartItem.ItemImage}}" alt="{{cartItem.ItemName}}" />
                                </div>
                                <div class="col-xl-3 col-lg-3 col-sm-12 v-center">
                                    <h5>{{cartItem.ItemName}}</h5>
                                </div>
                                <div class="col-xl-3 col-lg-3 col-sm-12 v-center" id="BestSellers">
                                    <div class="card-price-org">₹{{cartItem.WithDiscountItemPrice}}</div>
                                    <div class="card-price-dis">₹{{cartItem.OriginalPrice}}</div>
                                    <div class="card-price-dis-per">{{cartItem.DiscountPercentage}}% off</div>
                                </div>
                                <div class="col-xl-2 col-lg-2 col-sm-12 v-center">
                                    <%--<h5>{{cartItem.ItemCount}}</h5>--%>
                                    <div ng-if="cartItem.ItemCount !== 0" class="card-cart">
                                        <div class="card-cart-count">
                                            <div class="card-cart-count-child1" ng-click="AddToCart(cartItem.CategoryID,cartItem.ItemID,'Minus', cartItem.ItemCount, cartItem.CartID)"><i class="fa fa-minus"></i></div>
                                            <div class="card-cart-count-child2">{{cartItem.ItemCount}}</div>
                                            <div class="card-cart-count-child3" ng-click="AddToCart(cartItem.CategoryID,cartItem.ItemID, 'Plus', cartItem.ItemCount, cartItem.CartID)"><i class="fa fa-plus"></i></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-xl-2 col-lg-2 col-sm-12 v-center">
                                    <h5>₹ {{cartItem.TotalPrice}}</h5>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-xl-4 col-lg-4 col-sm-12">
                    <div class="cart-cal-box">
                        <%-- <div>
                           <h6>Select Address</h6>
                           <select class="" aria-label="Default select example">
                                <option value="1">One-twoooo</option>
                                <option value="2">Two</option>
                                <option value="3">Three</option>
                            </select>
                            <select class=""
                                ng-options="color.AddressUserName for color in addressList"
                                aria-label="Default select example">
                                <option ng-repeat="item in addressList" value="{{item.AddressID}}" >{{item.AddressUserName}}</option>
                            </select>
                        </div>--%>
                        <div id="#BestSellers">
                            <h6>Cart Total</h6>
                            <div class="cart-total-card-items">
                                <div>Cart Subtotal</div>
                                <div>₹{{CartSubTotal}}</div>
                            </div>
                            <div class="cart-total-card-items">
                                <div>Discount</div>
                                <div style="color: red">₹{{CartDiscount}}</div>
                            </div>
                            <div class="cart-total-card-items">
                                <div><b>Cart Total</b></div>
                                <div><b>₹{{CartTotal}}</b></div>
                            </div>
                            <div class="cart-apply-btn">
                                <button type="button" class="btn default-primary-btn" ng-click="DeleteUserAddress()">Apply</button>
                            </div>
                        </div>
                    </div>
                </div>
